package com.search_service.model.utils.cache;

import com.search_service.model.exceptions.GeneralFormatException;
import com.search_service.model.exceptions.TypeError;
...
public class LocalCacheContext<T> {
    ...
    private final LinkedHashMap<Field, String> columnNames;
    private final Map<Field, Method> setters;
    private final Map<Field, Method> getters;

    private LocalCacheContext(Class<T> tClass, Map<String, Class<?>> fieldTypes, Map<String, List<String>> fieldSynonyms, List<String> classSynonyms, LinkedHashMap<Field, String> columnName, Map<Field, Method> setters, Map<Field, Method> getters) {
    ...
    static <T> LocalCacheContext<T> create(Class<T> tClass) {
        ...
        if (excelClass != null) {
            return new LocalCacheContext<>(tClass, readFieldTypes(annotatedFields), readSynonymFields(annotatedFields), readSynonymsClass(excelClass), readColumnName(annotatedFields), getSettersFields(annotatedFields, tClass), getGettersFields(annotatedFields, tClass));
        } else {
    ...
    private static Map<String, List<String>> readSynonymFields(List<Field> fields) {
        return fields.stream().collect(Collectors.toMap(Field::getName, field ->
                Arrays.stream(field.getAnnotation(ExcelColumn.class)
                        .synonyms()).map(String::toLowerCase).toList()));
    }

    private static Map<String, Class<?>> readFieldTypes(List<Field> fields) {
        return fields.stream().collect(Collectors.toMap(Field::getName, Field::getType));
    }
