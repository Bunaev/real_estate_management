...
import com.search_service.model.exceptions.*;
import com.search_service.model.utils.cache.GlobalCacheManager;
import com.search_service.model.utils.cache.LocalCacheContext;
import com.search_service.model.utils.cache.SynonymResolver;
...
public class ExcelUtils {
    ...
    private static <T> List<T> readSheetFromWorkbook(Sheet sheet, LocalCacheContext<T> context) {
        ...
        if (!validateSheet(header, context)) {
            throw new GeneralFormatException(TypeError.FIELDS_NOT_MATCH, "Строки в книге: " + sheet.getSheetName() + " не совпадают с необходимыми.\nПроверьте аннотации или содержание файла.");
        } else {
    ...
    private static <T> T getParse(Class<T> type, String resultValue) {
        return (T) switch (type.getSimpleName()) {
            case "String" -> resultValue;
            case "Double" -> Double.parseDouble(resultValue);
            case "Float" -> Float.parseFloat(resultValue);
            case "Integer" -> Integer.parseInt(resultValue);
            case "Boolean" -> Boolean.parseBoolean(resultValue);
            default -> {
                if (type.isEnum()) {
                    try {
                        Method method = type.getMethod("parse", String.class);
                        yield method.invoke(null, resultValue);
                    } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException ex) {
                        yield Enum.valueOf((Class<Enum>) type, resultValue.toUpperCase());
                    }
                } else yield null;
            }
        };
    ...
    private static void createHeader(Sheet sheet, LinkedHashMap<Field, String> fields) {
        if (fields.isEmpty()) {
